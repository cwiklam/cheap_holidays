...
module Itaka
  class Fetcher
    def initialize(agency:, page:, query: nil, max_pages: 100)
      @agency = agency
      @page = page.to_i
      @query = query.to_s
      @max_pages = max_pages
    ...
    def call
      ...
      return false unless @unlimited || @page <= @max_pages.to_i
      ...
      offers = ::Itaka::Scraper.new(html, base_url: @agency.url).offers
      offers.select! { |offer| offer[:name].to_s.downcase.include?(@query.downcase) } if @query.present?
    ...
    def enqueue_next_page_if_needed
      next_page = @page + 1
      return if !@unlimited && next_page > @max_pages.to_i
      ...
      ::TravelAgencies::ItakaSequentialFetchJob.perform_later(page: next_page, query: @query.presence, max_pages: @max_pages)
    end
